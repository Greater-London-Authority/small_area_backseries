...
mye_net_11 <- readRDS("input_data/intermediate/msoa11_flows.rds") # agh. I've gotten rid of the process used to create this. I'm just going to split out the final flows here from 85 into 90 and over. And then it's a to-do to recreate the msoa11 net flows with 90+ the whole way. Again, in my opinion once you've "unrolled" the cohort the data past the previous max cohort is useless so I don't think it greatly matters. 

age_lookup <- data.table(
  age_85 = c(0:84, rep(85, 6)),
  age_90 = c(0:90),
  to_divide = c(rep(1, 85), rep(6, 6))
)

setkey(mye_net_11, "age")
setkey(age_lookup, "age_85")

mye_net_11 <- age_lookup[mye_net_11, 
                        allow.cartesian = TRUE]

mye_net_11[, gross_flows := gross_flows/to_divide]

mye_net_11 <- mye_net_11[, -c("age_85", "to_divide")]

colnames(mye_net_11)[colnames(mye_net_11) == "age_90"] <- "age"
...
age_lookup <- data.table(
  age_75 = c(0:74, rep(75, 16)),
  age_90 = c(0:90),
  to_divide = c(rep(1, 75), rep(16, 16))
)
...
colnames(all_flows)[colnames(all_flows) == "age_90"] <- "age"
...
gross_flows_msoa[inflow <= 0, inflow := 0.1]
gross_flows_msoa[outflow <= 0, outflow := 0.1]
