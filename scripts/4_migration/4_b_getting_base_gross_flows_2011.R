...
## 3. getting outflows at msoa level
  ...
  ### 3.1. fixing up the msoa net flows dataset (later, do the opposite of this, split out 75 into 85) (actually, I am less sure that it's sensible to split it out now? TO DISCUSS). 
...
### then, we use the modelled approach from the ukmig tables to estimate how much of the flows are within the geography of question
...
scaling_factors <- scaling_factors[, .(population_lsoa = sum(population_lsoa), population_msoa = sum(population_msoa)),
                                   by = .(sex, age, year, lsoa11cd, msoa11cd)]
...
gross_flows_lsoa <- gross_flows_lsoa[, -c("scaling_factor", "net_flows")]
  ...
  ### 4.3. scaling total number of within msoa flows to lsoa (which will be the amount we need to allocate for each lsoa) and joining with the gross flows dataset # NOTE - I think it would be meaningful to get this all into one function. A fairly discrete, defineable thing. 
...
gross_flows_lsoa <- gross_flows_lsoa[, c("lad23cd", "msoa11cd", "msoa11nm", "lsoa11cd", "year", "sex", "age", "moved_within_msoa", "population_lsoa", "inflow", "outflow")] # need to keep lsoa population, to get msoa21 population estimate later for estimating the within msoa21cd flows to be subtracted
...
## 5. getting msoa21 flows

  ### 5.1. aggregating lsoa11 up to msoa21
...
gross_flows_msoa <- aggregate_geographies_weighted(data = gross_flows_lsoa, lookup = lsoa11_msoa21_weighted, # TO CHECK - pretty sure that this function is right, but double check just in case. It's a new function. 
                                                   geog_from_data = "lsoa11cd", geog_from_lookup = "lsoa11cd", geog_to_lookup = "msoa21cd", # A to-do. I've estimated msoa21cd population, when I can just get it directly. I don't think this will make much difference, but still...
                                                   count_names = c("population_lsoa", "inflow", "outflow"))

colnames(gross_flows_msoa)[colnames(gross_flows_msoa) == "population_lsoa"] <- "population_msoa"

  ### 5.2. subtracting the within msoa flows, and extending the series to go up to the set max year
gross_flows_msoa[, moved_within_msoa := estimate_moved_within(population_msoa)]

gross_flows_msoa <- scale_down_total_variable(dataset = gross_flows_msoa, # also to double check. is it definitely within msoa flows that we want to subtract? I think so, but must be sure
                                              variable_to_get_age_distribution = "inflow", variable_to_scale_down = "moved_within_msoa",
                                              categories_to_keep = c("msoa21cd", "year"))

gross_flows_msoa <- scale_down_total_variable(dataset = gross_flows_msoa,
                                              variable_to_get_age_distribution = "outflow", variable_to_scale_down = "moved_within_msoa",
                                              categories_to_keep = c("msoa21cd", "year"))

gross_flows_msoa[, inflow := inflow - moved_within_msoa_scaled_by_inflow]
gross_flows_msoa[, outflow := outflow - moved_within_msoa_scaled_by_outflow]


gross_flows_msoa <- gross_flows_msoa[, c("msoa21cd", "year", "sex", "age", "inflow", "outflow")]


## 6. fixing up and saving the gross flows file
file_path <- paste0("input_data/intermediate/base_gross_flows_msoa21cd_2011_", max_year, ".rds")

saveRDS(object = gross_flows_msoa,
        file = file_path)
