...
ukmig_msoa <- fread("input_data/raw/ukmig008_msoa.csv")
  ...
  ### 1.2. estimated 2011 MYE net flows
mye_net_11 <- readRDS("input_data/intermediate/msoa11_flows.rds") 

  ### 1.3. msoa and lsoa 2011
msoa_pop <- fread(msoa11_population_path)
lsoa_pop <- fread(lsoa11_population_path)
...
mye_net_11 <- mye_net_11[, c("msoa11cd", "year", "age", "sex", "net_flows")]
...
gross_flows[, outflow := inflow - net_flows]  
...
gross_flows_lsoa <- gross_flows_lsoa[, c("msoa11cd", "msoa11nm", "lsoa11cd", "year", "sex", "age", "moved_within_msoa", "population_lsoa", "inflow", "outflow")] # need to keep lsoa population, to get msoa21/ward22 population estimate later for estimating the within msoa21cd flows to be subtracted
...
saveRDS(object = gross_flows_lsoa,
        file = "input_data/intermediate/base_gross_flows_lsoa11_2011.rds")
