## carrying out the iterative proportional fitting process to create single year of age death data on lsoa21 boundaries
...
## 1. reading in the data - margins and seed table completed in the previous script - and quick cleaning up
  ...
  ### 1.1. reading in data
lsoa_seed <- readRDS(paste0("input_data/intermediate/lsoa21_seed", "_to_", max_year, ".rds"))

lad_deaths_mar <- readRDS(paste0("input_data/intermediate/lad_deaths_margin_lsoa21", "_to_", max_year, ".rds"))

lsoa_deaths_mar <- readRDS(paste0("input_data/intermediate/lsoa21_deaths_margin", "_to_", max_year, ".rds"))

  ### 1.2. rearranging the columns - makes it slightly easier to line up the seed and margin dimensions
  ### and dropping gss_type - matching the dimensions for IPF is awkward enough without adding an extra column you can do without
lsoa_seed <- lsoa_seed[, c("gss_code", "lsoa21cd", "year", "sya", "sex", "deaths")]

lsoa_deaths_mar <- lsoa_deaths_mar[, c("gss_code", "lsoa21cd", "year", "sex", "deaths")]

lad_deaths_mar <- lad_deaths_mar[, c("gss_code", "year", "age", "sex", "value")]
...
ipf_est <- function(input_geog){
  ...
  lsoa_seed_sel <- lsoa_seed[gss_code == input_geog, ]
  
  lsoa_deaths_mar_sel <- lsoa_deaths_mar[gss_code == input_geog, ]
  
  lad_deaths_mar_sel <- lad_deaths_mar[gss_code == input_geog, ]
...
all_geogs <- lsoa_seed[, unique(gss_code)]
...
colnames(fitted_deaths) <- c("lsoa21cd", "year", "age", "sex", "deaths", "gss_code")

fitted_deaths <- fitted_deaths[, c("lsoa21cd", "year", "age", "sex", "deaths")] # getting rid of lad


## 4. saving the dataset
saveRDS(object = fitted_deaths,
        file = paste0("input_data/intermediate/fitted_lsoa21_deaths", "_to_", max_year, ".rds"))
