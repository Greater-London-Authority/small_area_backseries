...
## 1. reading in the data - margins and seed table completed in the previous script - and quick cleaning up
  ...
  ### 1.1. reading in data
lsoa_seed <- readRDS("input_data/intermediate/lsoa11_seed.rds")

lad_deaths_mar <- readRDS("input_data/intermediate/lad_deaths_margin_lsoa11.rds")

lsoa_deaths_mar <- readRDS("input_data/intermediate/lsoa11_deaths_margin.rds")

  ### 1.2. rearranging the columns - makes it slight easier to line up the seed and marginal dimensions
lsoa_seed <- lsoa_seed[, c("lad22cd", "lsoa11cd", "year", "sya", "sex", "deaths")]

lsoa_deaths_mar <- lsoa_deaths_mar[, c("lad22cd", "lsoa11cd", "year", "sex", "deaths")]
